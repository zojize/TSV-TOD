...
import type { RoundReport, TestOrder } from "./Types.js"
import { VitestFilesList } from "./Types.js"
...
export const RunProject = ({
  ...
  Effect.gen(function*() {
    ...
    const decodedVitestFiles = yield* Schema.decode(VitestFilesList)(rawVitestListOutput)
    // TODO: add a filter option to cli
    const vitestFiles = Option.match(filter, {
      onNone: () => decodedVitestFiles,
      onSome: (filter) => decodedVitestFiles.filter((f) => f.file.includes(filter))
    })
    ...
    for (const { file, i } of vitestFiles.map((x, i) => ({ ...x, i }))) {
      ...
      if (hasFlaked) {
        ...
        // TODO: double check that the flakiness is due to order and not just random failure
        // TODO: classifier
      }
    }
